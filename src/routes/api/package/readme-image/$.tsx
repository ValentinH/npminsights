...
import { OgBarsChart } from '~/components/OgBarsChart';
import { SIX_HOURS_IN_SECONDS } from '~/utils/consts';
import { getPackageInsights } from '~/utils/npm-api';
import { NpmDailyDownloads } from '~/utils/npm-api/types';

export const Route = createFileRoute('/api/package/readme-image/$')({
  server: {
    handlers: {
      GET: async ({ params }: { params: { _splat: string } }) => {
        const packageName = params._splat || '';
        if (!packageName) {
        ...
        const allMonthlyDownloads = data.allDailyDownloads.reduce<NpmDailyDownloads[]>(
          (acc, curr) => {
